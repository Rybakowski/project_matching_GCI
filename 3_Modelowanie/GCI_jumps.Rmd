...
vector <- c(10,-4,-3.5,10,-3,5,10,10,10,10)
...
test_everything <- function(variable){
	...
	list_to_save <- list(all_df, all_df_summary)
	writexl::write_xlsx(list_to_save, paste0("results_2024_01_05/jumps_20240106_",variable,"_yham.xlsx"))
...
filars_only <- vars_to_test_col[vars_to_test_col %>% str_detect("GCI")]
final_results <- future_map(  , safely(test_everything), .progress = TRUE,
			  .options = furrr_options(seed = 123))
print("finish")
...
results_files <- list.files("results_2024_01_05") 
...
final_results_read <- paste0("results_2024_01_05\\", results_files) %>% 
	map(.f = function(x){
		...
		to_return <- list(
			...
			Sheet2 = sheet2,
			name = x %>% 
				str_remove("results_2024_01_05\\\\") %>% 
				str_remove("20240106_") %>%
				str_remove("jumps_") %>%
