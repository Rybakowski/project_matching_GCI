...
```{r countries_treated}
filter_jumps <- treated_columns(wf_EOSQ057)
dict_cou %>% 
...
visual_jumps(wf_EOSQ057)
```

```{r Panel visualization}
visual_panel(wf_EOSQ057)
```
...
data_model <- data_to_model(wf_EOSQ057)
...
test_everything <- function(variable){
	...
	results_list <- map(vars_to_test, test_one_variable)
	all_df <- tibble(results = results_list) %>%
			mutate(variable = names(results)) %>%
			unnest(cols = c(results))
	all_df_summary <- all_df %>%
...
# vars_to_test_col <- vars_to_test[1:2]


plan(multisession, workers = 8)
future_map(vars_to_test_col, safely(test_everything), .progress = TRUE,
			  .options = furrr_options(seed = 123))
print("finish")
